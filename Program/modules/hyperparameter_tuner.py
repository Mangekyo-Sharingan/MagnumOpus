...
class HyperparameterTuner:
    ...
    def _create_tuning_subset(self, n_samples=2000):
        ...
        if n_samples > total_size:
            ...
            print(f"⚠️  Requested samples ({n_samples}) exceeds dataset size. Using all {total_size} samples.")
        ...
        print(f"✓ Tuning set created: {len(train_indices)} train, {len(val_indices)} validation")
    ...
    def grid_search(self, param_grid=None, n_samples=2000, save_results=True):
        ...
        if total_combinations > 200:
            print(f"\n⚠️  WARNING: {total_combinations} combinations will take a long time!")
        ...
        for idx, (batch_size, epochs, lr) in enumerate(param_combinations, 1):
            ...
            print(f"⏱️  Progress: {idx}/{total_combinations} | Elapsed: {self._format_time(elapsed)} | ETA: {self._format_time(remaining)}")
    ...
    def _print_results_summary(self, total_time):
        ...
        print(f"\n⏱️  Total search time: {self._format_time(total_time)}")
