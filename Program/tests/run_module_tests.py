...
# Add the parent directory (Program) and modules directory to the path
...
class ModuleTester:
    ...
    def run_all_tests(self):
        ...
        for module_name in self.modules_to_test:
            ...
            try:
                ...
                print(f"[CRITICAL] CRITICAL ERROR testing {module_name}: {e}")
                traceback.print_exc()
                self.test_results[module_name] = {
                    'status': 'FAIL',
                    'success': False,
    ...
    def _test_module(self, module_name):
        ...
        try:
            ...
            print(f"[OK] Successfully imported {module_name} module")
            ...
            if hasattr(module, test_function_name):
                ...
                print(f"[OK] Found test function: {test_function_name}")

                # Run the test
                result = test_function()

                if result:
                    print(f"[PASS] {module_name.upper()} MODULE TEST: PASSED")
                else:
                    print(f"[FAIL] {module_name.upper()} MODULE TEST: FAILED")

                return result
            else:
                print(f"[WARN] No test function found for {module_name}")
